...
def custom_loss(y_true, y_pred):
    ...
    PS_true_R = power2D(y_true_R)
    PS_true_G = power2D(y_true_G)
    PS_true_B = power2D(y_true_B)
    
    PS_pred_R = power2D(y_pred_R)
    PS_pred_G = power2D(y_pred_G)
    PS_pred_B = power2D(y_pred_B)
...
for i in [0,1,2]:
    ...
    PS_label = power2D(y_test[0,:,:,i])
    PS_pred = power2D(preds[0,:,:,i])
    ...
    l += 1
plt.savefig('power_spectrum_hist_withlog_70000_Adam.png',bbox_inches='tight')
